...
class MLPClassifier(object):
    def __init__(self,**kwargs):
        ...
        self.__initialize_weights(self.__Weight_init)
    ...
    def __initialize_weights(self,initialization):
    ...
    def fit(self,X,Y,Xv,Yv):
        ...
        for i in range(self.__Num_epochs):
            ...
            self.__training_loss.append(np.sum(loss)/loss.shape[0])
            self.__epoch_list.append(i+1)
            self.__validation_loss.append(np.mean(self.__loss(Yv,self.__forward_pass(Xv)[0])))
            if abs(np.sum(loss)/loss.shape[0]-previous_loss)<self.__convergence_threshold:
                print('\nNeural Net has converged...\n')
                break
            previous_loss=np.sum(loss)/loss.shape[0]
